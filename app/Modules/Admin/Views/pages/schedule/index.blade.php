			...
			<div class="box">
	            <div class="box-header">
	              <div class="pull-right">
	              	<a href="{!!route('admin.schedule.create')!!}" class="btn btn-primary btn-xs"><i class="glyphicon glyphicon-plus"></i> Add New</a>
				...
				<div class="box-body">
				  ...
				  <table id="table-post" class="table table-bordered table-striped">
				    ...
				    <tbody>
						...
						<tr>
							<td >{!!$item->id!!}</td>
							<td><b><a href="{!!route('admin.schedule.edit',$item->id)!!}">{!!$item->date!!}</a></b></td>
							<td>{!!$item->location!!}</td>
							<td>{!!$item->centers->name_vi!!}</td>
							<td>{!!$item->scheduleable->title!!}</td>
							<td>
							<a href="{!!route('admin.schedule.edit', array($item->id) )!!}" class="btn btn-info btn-xs"> Edit </a>
							{!!Form::open(array('route'=>array('admin.schedule.destroy',$item->id),'method'=>'DELETE', 'class' => 'inline'))!!}
		...
		$(document).ready(function(){
			...
			$('#btn-count').click( function () {
				...
				alertify.confirm('You can not undo this action. Are you sure ?', function(e){
					if(e){
						$.ajax({
							'url':"{!!route('admin.schedule.deleteall')!!}",
